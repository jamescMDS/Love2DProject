bluePlayerData = {}

function bluePlayerData.Construct()

  bluePlayerData.runAnimationFile = '3 Dude_Monster/Dude_Monster_Run_6.png'
  bluePlayerData.runAttackAnimationFile = '3 Dude_Monster/Dude_Monster_Walk+Attack_6.png'
  bluePlayerData.attackAnimationFile = '3 Dude_Monster/Dude_Monster_Attack1_4.png'
  bluePlayerData.jumpAnimationFile = '3 Dude_Monster/Dude_Monster_Jump_8.png'
  bluePlayerData.idleAnimationFile = '3 Dude_Monster/Dude_Monster_Idle_4.png'
  bluePlayerData.dieAnimationFile = '3 Dude_Monster/Dude_Monster_Death_8.png'
  bluePlayerData.damageAnimationFile = '3 Dude_Monster/Dude_Monster_Hurt_4.png'

end


return bluePlayerData
